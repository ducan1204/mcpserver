...
# from save_documents_to_vector import insert_parents, DocumentChunk, insert_childs, DocumentChildChunk
import json


multilingual = 'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2' #384
vietnamsbert = 'keepitreal/vietnamese-sbert' #768
tokenizer = AutoTokenizer.from_pretrained(vietnamsbert)

class ParentChunk():
    def __init__(self, id, content, document_id = None):
        self.id = id
        self.content = content
        self.document_id = document_id

class ChildChunk():
    def __init__(self, id, content, parent_id, document_id = None, embedding = None):
        self.id = id
        self.content = content
...
#     listDocuments = [DocumentChunk(id=chunk.id, content=chunk.content) for chunk in splitted_parent_chunks]
